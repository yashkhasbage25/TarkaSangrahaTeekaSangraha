...
export class BooksLayout extends Component<BooksLayoutProps, BooksLayoutState> {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div className="fixed top-30 left-6 z-50">
          <div className="bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-3xl shadow-lg cursor-pointer transition-colors duration-200">
            <IconButton onClick={this.togglePane}>
              {isPaneOpen ? <CloseIcon /> : <MenuBookIcon />} 
            </IconButton>
            {isPaneOpen && (
              <div className="absolute top-16 left-0 w-56 bg-gray-200 shadow-2xl rounded-2xl z-40 animate-in slide-in-from-top-2 duration-200">
                <div className="overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-200 overflow-x-hidden p-2 m-2" style={{ maxHeight: '70vh' }}>
                    <Box component="ul" className="space-y-2">
                      {sectionItems.map((item, idx) => (
                        <button
                          key={makeSectionKey(item.bookTitle, item.title, item.sectionId)}
                          className="w-full text-left px-1 py-1 text-gray-700 hover:bg-gray-50 transition-colors duration-150 last:border-b-0 focus:outline-none focus:bg-blue-50"
                          onClick={() => {
        ...
        <Box className="flex bg-transparent" sx={{ whiteSpace: 'pre-line' }}>
          ...
          <React.Fragment>  
            <IconButton  
              ...
              className="fixed left-4 top-24 bg-white rounded shadow"  
            >  
